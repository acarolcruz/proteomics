# DDA noNorm Proteinas ----

# Carrega arquivo de dados .RData

ggplot(d,aes(gr,y,color=T))+ geom_boxplot()+
  labs(title="DDA noNorm - Quantificação de Proteina - y", x = "Método(Tecido)")


ggplot(d,aes(x = M, color = T, group =T, y = y)) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.y = mean, geom = "line") +
  labs(title="DDA noNorm Proteína - Perfis - y", x = "Método")


ggplot(d,aes(gr,log10(y),color=T))+ geom_boxplot()+
  labs(title="DDA noNorm - Quantificação de Proteina -  log10(y)  ", x = "Método(Tecido)")


ggplot(d,aes(x = M, color = T, group =T, y = log10(y))) +
  stat_summary(fun.y = mean, geom = "point") +
  stat_summary(fun.y = mean, geom = "line") +
  labs(title="DDA noNorm Proteína - Perfis  - log10(y)", x = "Método")
...
# Modelo Linear Misto Heteroscedastico devido ao MT ----
...
ggplot(drms,aes(gr,log10(y),color=T))+ geom_boxplot()+
  labs(title="DDA noNorm - Quantificação Proteína - log10(y)", x = "Método(Tecido)")

ggplot(drms,aes(gr,y_norm,color=T))+ geom_boxplot()+
  labs(title="DDA noNorm - Quantificação Proteína - y Normalizado", x = "Método(Tecido)")
...
plot(rep(1:2289),-log10(as.numeric(anova_pvalues[,4])))
...
plot(rep(1:2289),-log10(p.adjust.MT))
...
pdf("DDA noNorm Proteinas - Significance.pdf", width = 12, height = 8)
par(mfrow=c(4,6))
...
#mtext("SRM-Peptide", outer=TRUE,  cex=1, line=-1.2)
for (L in 13:18){
  log10.pvalue <- log10(as.numeric(pvalues2by2[[L]][,"p-valor"]))
  estimate <- as.numeric(pvalues2by2[[L]][,"Estimativa"])
  plot(estimate,-log10.pvalue, main = names(pvalues2by2)[L], col = ifelse(-log10.pvalue > p.bf & abs(estimate) > 2,"red","black"))
...
# Base para analises multivariadas
#Considerando somente os cados completos
...
n.S <- length(unique(X$S)) #numero de amostras
T.id <- c("NP", "P", "T", "pool")
...
mean.Total <- as.vector(colMeans(X[,4:1262])) #media de cada F
for (tis in T.id) {
  for (meth in M.id) {
    ...
    mean.TM <- as.vector(colMeans(X[idTM,4:1262])) #media das observações T = t e M = m para cada F
    mean.T <- as.vector(colMeans(X[idT,4:1262])) #media das observações T = t para cada F
...
ids <- c(rep(1:nrow(X)))
for (grus in ids) {
  SSTotal <- SSTotal+(t(X[grus,4:1262])-mean.Total)%*%t(t(X[grus,4:1262])-mean.Total)
}
dim(SSTotal)
...
netMT<- cbind(ynorm.homo[,1:3],matrix(0,80,1259))
dim(netMT)
T.id <- c("NP", "P", "T", "pool")
M.id <- c("ISD", "IGD", "OPD", "OFD")
for (tis in T.id) {
  for (meth in M.id) {
    ...
    mean.TM <- as.vector(colMeans(X[idTM,4:1262]))
    mean.T <- as.vector(colMeans(X[idT,4:1262]))
    difMT <- t(as.matrix(mean.TM-mean.T))
    netMT[which(netMT$Tc == tis & netMT$Mc ==  meth),c(4:1262)] <- rbind(difMT,difMT,difMT,difMT,difMT)
  }
}
...
pcnet <- prcomp(netMT[,4:1262])
pcnet
summary(pcnet)
plot(pcnet$x[,1],pcnet$x[,2], xlab = "PC1 (39,77%)", ylab = "PC2 (23,54%)", type = "n", xlim = c(-500,1000))
text(pcnet$x[,1],pcnet$x[,2],labels = as.factor(paste0(netMT$Tc,":",netMT$Mc)))
plot(pcnet$rotation[,1],pcnet$rotation[,2], xlab = "PC1 (39,77%)", ylab = "PC2 (23,54%)", type = "n", xlim = c(-0.1,1.0))
text(pcnet$rotation[,1],pcnet$rotation[,2], labels = as.character(ynorm$Fc[1:1259]))

# Grafos ----

S <- cov.wt(netMT[,4:1262], method = "ML")$cov
C <- cov2cor(S)
suffStat <- list(C = C,n = nrow(netMT))
indepTest <- gaussCItest

cpdag <- pc(suffStat, gaussCItest, p = ncol(netMT[,4:1262]),
            alpha = 0.01)
nodes(cpdag@graph) <- as.character(ynorm$Fc[1:1259])

# Passos necessarios para obter grafos conexos
A5 <- cpdag@graph		
A5 <- as(A5, "matrix")	
g5 <- graph.adjacency(A5, mode='directed')	
V(g5)$name <- as.character(ynorm$Fc[1:1259])
...
pdf("Grafos_DDA_protein_noNorm.pdf")
for(i in grafos){
